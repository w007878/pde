function [u] = linear_element(f, p, q, x_s)
    ...
    u = zeros(n + 1, 1);
    for i = 2:n
        func = @(t)(p(x(i-1) + h(i)*xi(i, t))/h(i) + h(i)*q(x(i-1)+h(i)*xi(i, t))*((1-xi(i, t))^2));
        K(i - 1, i - 1) = K(i - 1, i - 1) + integral(func, 0, 1);

        func = @(t)(p(x(i-1) + h(i)*xi(i, t))/h(i) + h(i)*q(x(i-1)+h(i)*xi(i, t))*(xi(i, t)^2));
    ...
    for i = 2:n
        ...
        b(i - 1) = b(i - 1) + h(i) * integrate(func, 0, 1);
    end
